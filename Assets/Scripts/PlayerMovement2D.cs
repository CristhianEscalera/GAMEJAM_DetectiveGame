...
public class PlayerMovement2D : MonoBehaviour
{
    ...
    private Transform staminaBarTransform;

    private Rigidbody2D rb;
    private Vector2 velocity = Vector2.zero;
    private bool facingRight = true;

    // --- Escaleras (CORREGIDO: Usar contador en lugar de booleano) ---
    private int laddersInContact = 0; // Cuenta cuántas escaleras están tocando al player
    private bool isClimbing = false;
    private float horizontalInput;
    private float verticalInput;
    private float defaultGravity;

    // Getter público para saber si está escalando
    ...
    private void Update()
    {
        ...
        // Control de escaleras: Ahora verifica si hay AL MENOS una escalera en contacto
        bool isOnLadder = laddersInContact > 0;
    ...
    // --- TRIGGERS CORREGIDOS: Incrementar/Decrementar contador ---
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.CompareTag("Escalera"))
        {
            ...
            // Solo resetear si ya no hay escaleras en contacto
            if (laddersInContact <= 0)
            {
                laddersInContact = 0; // Asegurar que no sea negativo
                isClimbing = false;
                rb.gravityScale = defaultGravity;
            }
        }
